...
// https://github.com/rubensworks/jest-rdf/blob/master/lib/matchers/toBeRdfIsomorphic.ts

function getNonBlankDiff<Q extends RDF.BaseQuad = RDF.Quad>(a1: Q[], a2: Q[]): Q[] {
  return a1.filter(
    quad =>
...
export function unGroundHashes<Q extends RDF.BaseQuad = RDF.Quad>(graph: Q[]): ITermHash {
  return hashTerms(uniqGraph(getQuadsWithBlankNodes(graph)), getGraphBlankNodes(graph), {})[1];
}

export function getBnodeDiff<Q extends RDF.BaseQuad = RDF.Quad>(
  receivedQuads: Q[],
  expectedQuads: Q[],
): { received: Record<string, Q[]>; expected: Record<string, Q[]> } {
